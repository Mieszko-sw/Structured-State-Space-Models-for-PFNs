...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Measure per-dataset inference time for Hybrid 8L, TabPFN, and Hydra."
    )
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--warmup-runs", type=int, default=1)
    parser.add_argument("--did", type=int, default=11)
    parser.add_argument(
        "--time-csv",
        default=os.path.join(
            "result_csvs", "per_dataset_inference_hybrid8l_tabpfn_hydra.csv"
        ),
    )
    parser.add_argument(
        "--score-csv",
        default=os.path.join(
            "result_csvs", "per_dataset_scores_hybrid8l_tabpfn_hydra.csv"
        ),
    )
