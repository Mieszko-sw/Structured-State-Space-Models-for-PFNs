...
CHECKPOINT_SERIES = {
    "alternating_hydra_tabpfn_12l_lr1e4": {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark predictive performance across alternating Hydra-TabPFN 12L checkpoints."
    )
    ...
    parser.add_argument("--skip-latest", action="store_true", help="Do not evaluate the latest callback checkpoint.")
    parser.add_argument(
        "--summary-csv",
        default=os.path.join("result_csvs", "alternating_hybrid_checkpoint_performance_summary.csv"),
    )
    parser.add_argument(
        "--dataset-csv",
        default=os.path.join("result_csvs", "alternating_hybrid_checkpoint_performance_by_dataset.csv"),
    )
...
def main():
    ...
    specs = checkpoint_specs(args.models, args.epochs, not args.skip_final, not args.skip_latest)
    summary_rows = []
    dataset_rows = []
