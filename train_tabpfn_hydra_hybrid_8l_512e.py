...
config["hybrid_layer_types"] = ["transformer", "hydra"] * 4
config["nlayers"] = len(config["hybrid_layer_types"])
config["lr"] = 1e-4
lr_name = f"{config['lr']:g}".replace(".", "p")
checkpoint_model_name = f"tabpfn_hydra_x4_hybrid_8_layers_512e_lr{lr_name}"
...
wandb_run_name = (
    f"{model_type} tabpfn-hydra-x4 {config['nlayers']}l {config['emsize']}e "
