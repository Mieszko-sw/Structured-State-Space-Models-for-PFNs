...
config["hybrid_layer_types"] = ["hydra", "transformer"] * 6
config["nlayers"] = len(config["hybrid_layer_types"])
config["lr"] = 1e-4
lr_name = f"{config['lr']:g}".replace(".", "p")
checkpoint_model_name = f"alternating_hydra_tabpfn_12_layers_512e_lr{lr_name}"
...
wandb_run_name = (
    f"{model_type} alternating hydra-tabpfn {config['nlayers']}l {config['emsize']}e "
