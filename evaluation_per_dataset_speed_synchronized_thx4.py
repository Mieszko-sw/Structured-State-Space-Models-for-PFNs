"""Synchronized per-dataset evaluation for the THx4 model."""
...
MODELS = {
    "THx4": {
        "path": (
            "tabpfn/models_diff/"
            "callback_tabpfn_hydra_x4_hybrid_8_layers_512e_"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Measure synchronized per-dataset inference speed and predictive "
            "performance for the recently trained THx4 model on the real "
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            "result_csvs",
            "per_dataset_speed_synchronized_thx4_raw.csv",
        ),
    )
    parser.add_argument(
        "--summary-csv",
        default=os.path.join(
            "result_csvs",
            "per_dataset_speed_synchronized_thx4_summary.csv",
        ),
    )
