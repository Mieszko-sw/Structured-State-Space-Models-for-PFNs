...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Measure synchronized inference speed for the original models plus "
            "the hydra_small.cpkt checkpoint on real OpenML datasets."
        )
    )
    ...
    parser.add_argument(
        "--raw-csv",
        default=os.path.join("result_csvs", "per_dataset_speed_synchronized_hydra_small_raw_15.csv"),
    )
    parser.add_argument(
        "--summary-csv",
        default=os.path.join("result_csvs", "per_dataset_speed_synchronized_hydra_small_summary_15.csv"),
    )
