...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Measure synchronized NanoTabPFN inference speed under its 2-class, "
            "5-feature, 150-row evaluation constraints."
        )
    )
    ...
    parser.add_argument(
        "--raw-csv",
        default=os.path.join("result_csvs", "per_dataset_speed_synchronized_nanotabpfn_raw_15.csv"),
    )
    parser.add_argument(
        "--summary-csv",
        default=os.path.join("result_csvs", "per_dataset_speed_synchronized_nanotabpfn_summary_15.csv"),
    )
...
def load_model(model_name, device):
    ...
    nano_model = NanoTabPFNModel(**checkpoint["model_config"])
    nano_model.load_state_dict(checkpoint["model_state_dict"])
    model = NanoTabPFNEvaluationWrapper(nano_model).to(device)
    config = checkpoint.get("evaluation_config", {})
    config.setdefault("bptt", 150)
    config.setdefault("eval_positions", [75])
    config.setdefault("max_num_classes", 2)
    config.setdefault("max_num_features", 5)
    model.eval()
