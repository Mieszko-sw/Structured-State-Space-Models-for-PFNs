"""Compute per-model mean inference time and metric from all_46.csv."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "each model in all_46.csv."
        )
    )
    parser.add_argument(
        "--input-csv",
        default="result_csvs/all_46.csv",
        help="Input per-dataset summary CSV.",
    )
    parser.add_argument(
        "--output-csv",
        default="result_csvs/all_46_model_means.csv",
...
def main():
    ...
    if invalid_row_count:
        print(f"Ignored {invalid_row_count} row(s) with nonnumeric values.\n")
