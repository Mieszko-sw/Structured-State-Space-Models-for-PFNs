"""Train an elastic six-physical-layer TabPFN with up to six core-stack loops.

The core executes cyclically as (A -> B -> C -> D -> E -> F)^d. Each update
uses the full d=6 trajectory (36 block applications) and one uniformly sampled
shortcut d in {1, 2, 3, 4, 5} (6, 12, 18, 24, or 30 block applications).
The loss combines full CE, shortcut CE, and detached full-to-short,
temperature-scaled KL distillation. This is the shortcut-distillation baseline;
it does not add LoopFormer time/step-size modulation inside the block.
"""
...
LOG_FILE_PATH = "elastic_looped_transformer_6physical_core6x.log"
...
checkpoint_model_name = "elastic_looped_transformer_6physical_core6x"
...
config["looped_core_layers"] = 6
config["looped_exit_layers"] = 0
config["looped_core_repeat_pattern"] = None

# Elastic shortcut-distillation objective. One shared core-stack depth is sampled per batch.
config["elastic_shortcut_training"] = True
config["elastic_max_loops"] = 6
config["elastic_shortcut_min_loops"] = 1
config["elastic_shortcut_ce_weight"] = 0.1
config["elastic_consistency_weight"] = 0.1
config["elastic_distillation_temperature"] = 2.0
config["lr"] = 1e-4
...
device = os.environ.get("TRAIN_DEVICE", "cuda:6")
...
# Save the elastic six-physical-layer, maximum-six-stack-loop TabPFN model
