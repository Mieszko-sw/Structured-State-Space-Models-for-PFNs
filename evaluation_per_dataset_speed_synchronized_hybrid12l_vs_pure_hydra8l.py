...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Compare synchronized per-dataset inference speed for Hybrid 12L and "
            "Pure Hydra 8L on the real OpenML datasets used by evaluation_script.py."
        )
    )
    ...
    parser.add_argument(
        "--raw-csv",
        default=os.path.join("result_csvs", "per_dataset_speed_synchronized_hybrid12l_vs_pure_hydra8l_raw.csv"),
    )
    parser.add_argument(
        "--summary-csv",
        default=os.path.join("result_csvs", "per_dataset_speed_synchronized_hybrid12l_vs_pure_hydra8l_summary.csv"),
    )
